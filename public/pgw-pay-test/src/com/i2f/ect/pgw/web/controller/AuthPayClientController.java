...
public class AuthPayClientController {
	
	public static final Logger logger = LoggerFactory.getLogger(AuthPayClientController.class);
	...
	public ModelAndView doPay(HttpServletRequest request) throws Exception{
		...
		String xmlString = ECTXmlUtil.mapToXml(map, ECTXmlUtil.CPREQ_AMSREQ);
	...
	@RequestMapping("/authpayPage")
	public ModelAndView goPayPage(){
		logger.info("进入快捷支付测试输入页面");
		ModelAndView model = new ModelAndView("authpay");
		return model;
	}
