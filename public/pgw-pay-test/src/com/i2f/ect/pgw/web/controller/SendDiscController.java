...
public class SendDiscController {
	...
	public ModelAndView doPay(HttpServletRequest request) throws UnsupportedEncodingException, IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchMethodException{
		...
		String xmlString = ECTXmlUtil.mapToXml(map, ECTXmlUtil.CPREQ_BCRES);
		logger.info("批量送盘接口测试发出报文：" + xmlString);
	...
	public ModelAndView senddo(){
		logger.info("进入批量代收付送盘");
		ModelAndView model = new ModelAndView("senddisc");
		return model;
	}
	 ...
	 * @throws Exception 
	 */
	@RequestMapping("/getSignedString")
	public @ResponseBody Object getSignedString(HttpServletRequest request)
			throws Exception {
		String preSignStr = IOUtils.toString(request.getReader());
		logger.info("传入明文：" + preSignStr);
		String merchantId = "";
		String[] strList = preSignStr.split(",");
		for (String str : strList) {
			if (str.indexOf("merchantId") != -1) {
				merchantId = str.split("=")[1];
				break;
			}
		}
		preSignStr = preSignStr.replaceAll(",", "&");
		String signedString = new String(MerchantSignAndVerify.sign(preSignStr,
				merchantId));
		logger.info("返回的密文：" + signedString);
