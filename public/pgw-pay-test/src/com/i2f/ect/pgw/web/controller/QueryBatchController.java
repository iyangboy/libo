...
public class QueryBatchController {
	
	public static final Logger logger = LoggerFactory.getLogger(QueryBatchController.class);
	
	public static String LOCALHOST_URL = "https://www.tongtongcf.com:9204/pgw-pay/QueryBatch";
	...
	public ModelAndView doPay(HttpServletRequest request) throws UnsupportedEncodingException, IllegalAccessException, InstantiationException, InvocationTargetException, NoSuchMethodException{
		...
		String xmlString = ECTXmlUtil.mapToXml(map, ECTServerXmlUtil.CPRES_QBATREQ);
		logger.info("批量状态查询测试发出报文：" + xmlString);
	...
	public ModelAndView senddo(){
		logger.info("进入批量代收付查询接口");
		ModelAndView model = new ModelAndView("querybatch");
		return model;
	}
